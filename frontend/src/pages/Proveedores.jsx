...
export default function Proveedores() {
  ...
  const guard = (action, fn) => (...args) => {
    if (!can(usuario?.rol, 'proveedores', action)) { denyPermission(); return }
    fn(...args)
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editTarget) await updateProveedor(editTarget.id_proveedor, values)
      else            await createProveedor(values)
      handleClose(); reload()
  ...
  const handleConfirmDelete = async () => {
    ...
    try { await deleteProveedor(deleteTarget.id_proveedor); reload() }
  ...
  return (
    <div className="page">
      ...
      <Modal isOpen={modalOpen} onClose={handleClose} title={editTarget ? 'Editar proveedor' : 'Nuevo proveedor'}>
        <div className="form">
          <ErrorMessage message={submitError} />
          <FormField label="Nombre" error={errors.nombre}>
            <input name="nombre" value={values.nombre} onChange={handleChange} placeholder="Ej. Nike Distribution GT" />
          </FormField>
          <FormField label="Teléfono">
            <input name="telefono" value={values.telefono} onChange={handleChange} placeholder="Ej. 22001001" />
          </FormField>
          <FormField label="Email">
            <input name="email" type="email" value={values.email} onChange={handleChange} placeholder="Ej. ventas@proveedor.com" />
          </FormField>
      ...
      <ConfirmModal
        ...
        message={`El proveedor "${deleteTarget?.nombre}" será eliminado permanentemente.`}
